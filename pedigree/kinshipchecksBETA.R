...
### Edited 6/15/15:
## This file is a companion to kinship.checks.R but is rather messy.
# The important things are that I identified discrepancies in some of the fam names
# and sire IDs for individuals in F50-56. A description of the errors are below, and
# I have changed these IDs in the allData.txt file, the geno.samples file if necessary,
# and the ailpedforQTLrel.txt file. I have NOT gone back and changed IDs in the
# AI Line Info files, so if the pedigree were regenerated from those, you would need
# to go back and check these IDs again.
## The rest of the file is just code I used to make heat plots of the relationships
# between AIL mice from the kinship matrices. They'are experiments and not very
...
# Visualizing relationships between (a) mice with sibs that have been
# genotyped, (b) mice with no sibs that have been genotyped, and (c) a matrix of all
# non-sib mice - that is, siblings were removed so that the matrix contains one sib
# from each sib pair, and all mice without a sib.
